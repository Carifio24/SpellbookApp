    ...
    boolean getVerbalFilter(boolean b) { return b ? verbalFilter : notVerbalFilter; }
    boolean getSomaticFilter(boolean b) { return b ? somaticFilter : notSomaticFilter; }
    boolean getMaterialFilter(boolean b) { return b ? materialFilter : notMaterialFilter; }
        ...
        void setVerbalFilter(boolean f, boolean b) {
            if (f) {
                verbalFilter = b;
            } else {
                notVerbalFilter = b;
        ...
        void setSomaticFilter(boolean f, boolean b) {
            if (f) {
               somaticFilter = b;
            } else {
                notSomaticFilter = b;
        ...
        void setMaterialFilter(boolean f, boolean b) {
            if (f) {
                materialFilter = b;
            } else {
                notMaterialFilter = b;
        ...
        private void toggleFilter(Boolean f, BiConsumer<Boolean,Boolean> filterSetter, Function<Boolean,Boolean> filterGetter) {
            filterSetter.accept(f, !filterGetter.apply(f));
        }
        void toggleRitualFilter(boolean f) { toggleFilter(f, this::setRitualFilter, this::getRitualFilter); }
        void toggleConcentrationFilter(boolean f) { toggleFilter(f, this::setConcentrationFilter, this::getConcentrationFilter); }
        void toggleVerbalFilter(boolean f) { toggleFilter(f, this::setVerbalFilter, this::getVerbalFilter); }
        void toggleSomaticFilter(boolean f) { toggleFilter(f, this::setSomaticFilter, this::getSomaticFilter); }
        void toggleMaterialFilter(boolean f) { toggleFilter(f, this::setMaterialFilter, this::getMaterialFilter); }
...
//    <E extends Enum<E>> E[] getVisibleValues2(Class<E> enumType) { return getVisibleValues2(enumType, true, enumType, enumType::cast); }
    ...
    <E extends Enum<E> & Named> String[] getVisibleNames(Class<E> enumType, boolean b) { return getVisibleValues(enumType, b, String.class, E::getDisplayName); }
    <E extends Enum<E> & Named> List<String> getVisibleNamesList(Class<E> enumType, boolean b) { return Arrays.asList(getVisibleNames(enumType, b)); }
    ...
    int getMinBaseValue(Class<? extends QuantityType> quantityType) { return getMinValue(quantityType) * getMinUnit(quantityType).value(); }
    int getMaxBaseValue(Class<? extends QuantityType> quantityType) { return getMaxValue(quantityType) * getMaxUnit(quantityType).value(); }
    ...
    void setKnown(Spell s, Boolean known) { setProperty(s, known, (SpellStatus status, Boolean tf) -> status.known = tf); }
        ...
        JSONObject toJSON() throws JSONException {
            ...
            json.put(sort1Key, firstSortField.getDisplayName());
            json.put(sort2Key, secondSortField.getDisplayName());
            ...
            for (HashMap.Entry<Class<? extends Enum<?>>, Quartet<Boolean, Function<Object,Boolean>, String, String>> entry : enumInfo.entrySet()) {
                ...
                final JSONArray jsonArray = new JSONArray(getVisibleNames(cls, false));
