...
class Spellbook {
    ...
    static private final Map<Version, Collection<Source>> NEW_IN_VERSION = new HashMap<Version, Collection<Source>>() {{
    ...
    static Collection<Source> newSourcesForVersion(Version version) {
        return NEW_IN_VERSION.get(version);
    }

    static private Collection<Source> sourcesByCondition(Function<Version,Boolean> versionCondition) {
        return NEW_IN_VERSION.entrySet().stream().filter(entry -> versionCondition.apply(entry.getKey())).map(entry -> entry.getValue()).flatMap(Collection::stream).collect(Collectors.toList());
    }
